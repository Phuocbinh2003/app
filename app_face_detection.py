...
def sliding_window_detect(img, model, step_size=5, window_size=(100, 100)):  # Thay đổi kích thước cửa sổ
    boxes = []
...
if __name__ == "__main__":
    ...
    # Đọc ảnh kiểm tra
    test_img = cv2.imread('/content/drive/MyDrive/Mydrive/Githut/faces_and_non_faces_data/mat.jpg', cv2.IMREAD_GRAYSCALE)

    # Resize ảnh kiểm tra giữ nguyên cấu trúc
    test_img_resized = resize_image_aspect_ratio(test_img, target_size=240)

    # Phát hiện box chứa khuôn mặt
    boxes = sliding_window_detect(test_img_resized, knn_model)

    # Chọn box tốt nhất
    selected_boxes = non_max_suppression(boxes, overlap_threshold=0.3)

    # Vẽ các box lên ảnh
    result_img = draw_boxes(test_img_resized, selected_boxes)

    # Hiển thị kết quả
    cv2.imshow('Kết quả phát hiện khuôn mặt', result_img)  # Hiển thị ảnh với các box phát hiện
    cv2.waitKey(0)
